...
1. **로컬 규칙 기반 실시간 리포트**: 수집된 최신 기사를 분석하여 일간/주간/월간 단위의 트렌드 요약 리포트를 자동으로 제공합니다.
2. **해외/국내 실시간 크롤링 IT 피드**: 해외와 국내 RSS 피드를 나누어 수집하고, 권역별 요약 및 기사 원문 이동을 제공합니다.
...
├── app.js                       # Vue 기반 대시보드 웹 애플리케이션 메인 로직
├── index.html                   # Vue 앱 마운트 및 대시보드 웹 메인 화면
├── style.css                    # 대시보드 스타일 시트
├── crawler.py                   # IT/AI 뉴스 수집 및 로컬 요약 엔진 (Python)
...
### 1. 백엔드 크롤러 실행 (Python)
크롤러는 RSS 피드 수집과 로컬 규칙 기반 요약을 통해 `crawled_data.js`를 동적으로 생성합니다. 외부 생성형 AI API 키는 사용하지 않습니다.

현재 기본 수집 대상은 해외 8개, 국내 6개 RSS 피드입니다.

해외: TechCrunch AI, Hacker News, VentureBeat AI, Ars Technica AI, GitHub Engineering, Netflix TechBlog, Cloudflare Blog, AWS ML Blog

국내: Toss Tech, Kakao Tech, NAVER D2, LINE Engineering, Woowahan Tech, Daangn Tech
...
DB 기반 동적 사이트로 운영하려면 Cloudflare Pages Functions + D1을 사용합니다. 앱은 `/api/bootstrap`에서 D1 데이터를 먼저 읽고, API가 없으면 기존 정적 JS 데이터로 fallback합니다.

자세한 설정은 [docs/cloudflare-dynamic-deploy.md](docs/cloudflare-dynamic-deploy.md)를 참고하세요.
